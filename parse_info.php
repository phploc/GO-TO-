...
$DB_urls= $mysqli->query("SELECT id,url FROM urls WHERE category is NULL LIMIT 50");
...
function parse_addinf(&$mysqli,$url){
	...
	preg_match('@<span class="user-name">(.+?)</span>@i', $e, $name);
	$info['name']=htmlspecialchars_decode($name[1],ENT_QUOTES);
	$bool=preg_match('@<span class="price main">(.+?)</span>@s', $e, $matches);
	if($bool){
		preg_match_all('@[\d,A-z]+@',$matches[1],$r);
		foreach($r[0] as $v)
		$str.=$v;
		preg_match('@([,\d]+)(\D+)@', $str, $prise_val);
		$info['prise']=str_replace(',','.',$prise_val[1]);
		$valute=$prise_val[2];
	}
	else{
		$info['prise']=0;
	...
	$mysqli->multi_query("INSERT INTO currency(valute) SELECT '{$esc_val}' FROM currency WHERE NOT EXISTS (SELECT 1 FROM currency WHERE valute = '{$esc_val}' LIMIT 1) LIMIT 1; SELECT * FROM currency WHERE valute='{$esc_val}' LIMIT 1 ;");
		if ($mysqli->error) printf("Errormessage: %s\n", $mysqli->error);
		$mysqli->next_result();
	...
	preg_match('@<img id="item-big-photo" src="(.+?)"@', $e, $img); //ссылка на фото
	$info['img']=$img[1];
	
	preg_match('@<div class="addition-word.*?">(.+?)</div>@s', $e, $text); //тело объявки
	$info['body']=trim(strip_tags($text[0]));
	$info['body']=htmlspecialchars_decode($info['body'],ENT_QUOTES);
	
	preg_match('@<div class="item">Город: <span class="value">(.+?)</span></div>@', $e, $city);
	$town=$city[1];
	
	$esc_city = $mysqli->real_escape_string($town);
	$mysqli->multi_query("INSERT INTO cities(town) SELECT '{$esc_city}' FROM cities WHERE NOT EXISTS (SELECT 1 FROM cities WHERE town = '{$esc_city}' LIMIT 1) LIMIT 1; SELECT * FROM cities WHERE town='{$esc_city}' LIMIT 1;");
		if ($mysqli->error) printf("Errormessage: %s\n", $mysqli->error);
		$mysqli->next_result();
	...
	$flag=false;
	preg_match('@<div class="breadcrumbs">(.+?)</div>@s', $e, $town_cat);  // город и категория
	preg_match_all('/title="(.+?)"/', $town_cat[1], $town_cat, PREG_PATTERN_ORDER, 0);  
	$id_cat=0;
	...
	elseif($flag==true){
		...
		$esc_moth = $mysqli->real_escape_string($id_cat);					//обновление
		$mysqli->multi_query("INSERT INTO categories(category,mother) SELECT '{$esc_cat}','{$esc_moth}' FROM categories WHERE NOT EXISTS (SELECT 1 FROM categories WHERE category = '{$esc_cat}' LIMIT 1) LIMIT 1; SELECT * FROM categories WHERE category='{$esc_cat}' LIMIT 1;");
		if ($mysqli->error) printf("Errormessage: %s\n", $mysqli->error);
		$mysqli->next_result();
